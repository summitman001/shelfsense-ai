...
export default function CoreSystem() {
  ...
  return (
    <section id="system" className="py-32 relative overflow-hidden bg-[#f0f9ff]" ref={ref}>
      <div
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        >
          <p className="text-xs text-[#0ea5e9] font-mono uppercase tracking-widest mb-4 font-semibold">Temel Sistem — The Root</p>
          ...
          {steps.map((s, i) => (
            <div key={s.title} className="relative flex items-stretch">
              <motion.div
                ...
                className="bg-white rounded-2xl p-8 flex-1 relative overflow-hidden group border border-slate-100 hover:border-sky-200 hover:shadow-lg transition-all duration-300"
              >
                <div className="absolute top-0 left-0 right-0 h-1 rounded-t-2xl" style={{ background: `linear-gradient(90deg, transparent, ${s.color}, transparent)` }} />
                ...
                <div className="text-xs font-mono mb-2 px-2 py-1 rounded-full inline-block" style={{ background: `${s.color}12`, color: s.color }}>
                  {s.detail}
                </div>

                <h3 className="text-xl font-bold text-[#0c172a] mb-3 mt-3">{s.title}</h3>
                <p className="text-slate-400 text-sm leading-relaxed">{s.desc}</p>

                <div className="text-3xl font-black mt-6 opacity-8" style={{ color: s.color, opacity: 0.08 }}>
                  0{i + 1}
                </div>
              </motion.div>
        ...
        <motion.div
          ...
          className="bg-white rounded-2xl p-8 border border-sky-100 shadow-sm"
        >
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            <div className="flex-1">
              <h3 className="text-2xl font-bold mb-3 text-[#0c172a]">Neler tespit ediliyor?</h3>
            ...
            <div className="lg:w-72 rounded-xl overflow-hidden border border-slate-100 bg-slate-900">
              <div className="p-3 border-b border-white/5 flex items-center gap-2 bg-slate-800">
              ...
              <div className="p-4 space-y-1.5 font-mono text-[11px]">
                {[
                  ...
                  { t: "00:01.891", msg: "Süt 1L → stok:67 → NORMAL", c: "#0ea5e9" },
                  { t: "00:02.103", msg: "Ekmek → stok:3 → KRİTİK", c: "#ef4444" },
                  { t: "00:02.440", msg: "Kampanya tetiklendi → Kaşar", c: "#6366f1" },
                  { t: "00:02.891", msg: "Geofence → 12 kullanıcı tespit", c: "#06b6d4" },
                  { t: "00:03.200", msg: "Fiyat güncellendi → -15%", c: "#0ea5e9" },
                ].map((l, i) => (
